    ...
    if(isset($_POST['action'])) {
        ...
        switch($rqst->action) {
            ...
            case 'updtRmk':
                ...
                $query = "select count(*) as count,COUNT(IF(zflag = 'Q', 1, NULL)) as qccnt from veh_item where pass_id = ?";
                $param = array(substr($rqst->objky,0,10));
                $item = $util->execQuery($query,$param,1);
                if($item->count == $item->qccnt) {
                    $rqst->pstatus = 'QC';
                    $query = "update veh_pass set cstat = ? where objky = ?";
                    $param = array( $rqst->pstatus,substr($rqst->objky,0,10));
                    $items  = $util->execQuery($query,$param);
                    $_SESSION['status'] = 'Quality Comments update complete';
                }    
                break;
            case 'updtQty' :
                    ...
                    <table class="table table-striped table-bordered">
                        <tr>
                            ...
                            <td colspan="2" class="text-right">
                                ...
                                <input type="hidden" name="objky" id="objky" value="<?php echo "{$item->objky}"; ?>">
                                <input type="hidden" name="bsart" id="bsart" value="<?php echo "{$item->bsart}"; ?>">
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="text" name="matnr" id="matnr" class="form-control" value="<?php echo $item->matnr;?>"    readonly >
                            </td>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <select name="lgort" id="lgort" class="form-control select2" required disabled >
                        ...
                        <tr>
                            ...
                            <input type="text" name="zppbg" id="zppbg" class="form-control text-right"   onblur="calcBag();" value="<?php echo $item->zppbg; ?>"    readonly >
                            </td>
                            <td>
                                <label for="">Jute Bags</label>
                                <input type="text" name="zjtbg" id="zjtbg" class="form-control text-right"  onblur="calcBag();"  value="<?php echo $item->zjtbg; ?>"    readonly >
                            </td>
                            <td>
                                <div class="row">
                                    <div class="col-sm-8">
                                    ...
                                    <div class="col-sm-4">
                                        ...
                                        <select name="zirej" id="zirej" class="form-control">
                                            <option value=""  <?php if($item->zirej == "" ) { echo 'selected';} ?>>       </option>
                                            <option value="X" <?php if($item->zirej == "X") { echo 'selected';} ?>>Reject </option>
                                        </select>
                                    </div>
                                </div>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input type="text" name="zgqty" id="zgqty" class="form-control text-right"   onblur="calcQty();" value="<?php echo $item->zgqty; ?>"    readonly required>
                            </td>
                            <td>
                                <label for="">Accepted Qty</label>
                                <input type="text" name="zaqty" id="zaqty" class="form-control text-right"   onblur="calcQty();" value="<?php echo $item->zaqty; ?>"    readonly required>
                            </td>
                            <td>
                        ...
                        <tr>
                            ...
                            <td colspan="2">
                                <label for="">Unload Superwiser</label>
                                <input type="text" name="zswsr" id="zswsr" class="form-control "   value="<?php echo $item->zswsr; ?>"    readonly >
                            </td>
                        </tr>       
...
?>
    <script>
        $(document).ready(function(){
            ...
            switch(zflag) {
                ...
                case 'A' :
                    ...
                    $('#zswsr').prop('readonly', false); 
                    if (bsart == 'ZRAW' ) {
                        if (matnr == '100400') {
                            $('#zppbg').prop('readonly', true);
                            $('#zjtbg').prop('readonly', true);
                        } else {
                        ...
                        $('#zppbg').prop('readonly', true);
                        $('#zjtbg').prop('readonly', true);
                        if (bsart == 'ZFUL' || matnr == '100400') {
                            $('#zgqty').prop('readonly', true);
                            $('#zaqty').prop('readonly', true);
                        } else {
                            $('#zgqty').prop('readonly', false);
                            $('#zaqty').prop('readonly', false);
                        }
                    }
                    break;
                case 'U' :
        ...
        function calcQty() {
            ...
            let zgqty = $('#zgqty').val();
            let zaqty = $('#zaqty').val();
            let zrqty = Number(zgqty) - Number(zaqty);
            $('#zrqty').val(zrqty);
